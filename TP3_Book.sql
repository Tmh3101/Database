...
SELECT T.TUA, TMP.SOSACH FROM ( SELECT NT, COUNT(NS) SOSACH FROM SACH
                                GROUP BY NT
...
SELECT T.TUA, TMP.SLS FROM (
    SELECT NT, COUNT(NS) SLS FROM SACH
    GROUP BY NT
) TMP
JOIN TACPHAM T ON TMP.NT = T.NT;

-- 19. Tìm số lầm mượn sách của mỗi độc giả theo năm
SELECT TMP.NAM, D.TEN, TMP.SLM FROM (
    SELECT ND, EXTRACT(YEAR FROM NGAYMUON) NAM, COUNT(*) SLM FROM MUON
...
SELECT T.TUA, COUNT(S.NS) SLS FROM SACH S
JOIN TACPHAM T ON S.NT = T.NT
GROUP BY T.TUA
HAVING COUNT(S.NS) >= ALL (
    SELECT COUNT(NS) SLS FROM SACH
    GROUP BY NT
);

-- 22. Tên nhà sản xuất xuất bản nhiều sách nhất
SELECT NXB, COUNT(NS)SOSACH FROM SACH
GROUP BY NXB
HAVING COUNT(NS) > = ALL (
    SELECT COUNT(NS) SLS FROM SACH
    GROUP BY NXB
...
SELECT S.NT, T.TUA, COUNT(DISTINCT M.ND) SNM FROM TACPHAM T
JOIN SACH S ON T.NT = S.NT
JOIN MUON M ON M.NS = S.NS
GROUP BY S.NT, T.TUA
HAVING COUNT(DISTINCT M.ND) >= ALL (
    SELECT COUNT(DISTINCT M.ND) FROM SACH S
    JOIN MUON M ON M.NS = S.NS
    GROUP BY S.NT
);
...
SELECT D.TEN, M.ND, COUNT(DISTINCT S.NT) STP FROM MUON M
JOIN SACH S ON M.NS = S.NS
JOIN DOCGIA D ON M.ND = D.ND
GROUP BY D.TEN, M.ND
HAVING COUNT(DISTINCT S.NT) >= ALL (
    SELECT COUNT(DISTINCT S.NT) FROM MUON M
    JOIN SACH S ON M.NS = S.NS
    GROUP BY M.ND
);
...
-- CUNG LA CAU 30 NHUNG LA CACH THU 2:
SELECT D.HO, D.TEN FROM (
    SELECT ND FROM DOCGIA
    MINUS
    SELECT DISTINCT ND FROM MUON
) TMP
JOIN DOCGIA D ON TMP.ND = D.ND;
...
-- CUNG LA CAU 31 NHUNG LA CACH THU 2:
SELECT DISTINCT S.NXB FROM (
    SELECT NS FROM SACH
    MINUS
    SELECT DISTINCT NS FROM MUON
) TMP
JOIN SACH S ON TMP.NS = S.NS;
